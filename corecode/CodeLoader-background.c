...
// Exit Status Flags
const char* operationIncomplete = "/tmp/CodeLoader.Background.operationIncomplete";
const char* operationComplete = "/tmp/CodeLoader.Background.operationComplete";
const char* safeStart = "/Library/CodeLoader/conf/bg/safeboot_boot";
...
int main() {
	status();
	if (exitCode == 0) {
		exitCode = system("/Library/CodeLoader/bin/exec_codeloader-background");
	}
	if (exitCode == 0) {
		printf("Operation Success.\n");
		system("echo \"I:BOOT_EXIT=CLEAN\" >> /tmp/CodeLoader.Background.Report");
	}else if (exitCode == 1) {
		printf("Safemode is enabled.\n");
		system("echo \"E:BOOT_EXIT=SAFEMODE\" >> /tmp/CodeLoader.Background.Report");
	}else if (exitCode == 2) {
		printf("Operation already finished.\n");
	}else if (exitCode == 3) {
		printf("Operation did not cleanly finished.\n");
		system("echo \"E:BOOT_EXIT=UNCLEAN_FINISH\" >> /tmp/CodeLoader.Background.Report");
	}
	exitCode = 0;
	return exitCode;
}
